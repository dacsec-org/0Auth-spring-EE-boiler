...
public interface UserRepository extends JpaRepository<User, Integer> , UserRepositoryCustom{
    ...
    @Query("SELECT u FROM UserPage u WHERE u.status = 1")
    Collection<User> findAllActiveUsers();
    
    @Query("select u from UserPage u where u.email like '%@gmail.com'")
    ...
    @Query("SELECT u FROM UserPage u WHERE u.status = ?1 and u.name = ?2")
    User findUserByStatusAndName(Integer status, String name);
    
    @Query("SELECT u FROM UserPage u WHERE u.status = :status and u.name = :name")
    ...
    @Query("SELECT u FROM UserPage u WHERE u.name like ?1%")
    User findUserByNameLike(String name);
    
    @Query("SELECT u FROM UserPage u WHERE u.name like :name%")
    User findUserByNameLikeNamedParam(@Param("name") String name);
    
    @Query(value = "SELECT * FROM users u WHERE u.name LIKE ?1%", nativeQuery = true)
    User findUserByNameLikeNative(String name);
    
    @Query(value = "SELECT u FROM UserPage u")
    List<User> findAllUsers(Sort sort);
    
    @Query(value = "SELECT u FROM UserPage u ORDER BY id")
    ...
    @Modifying
    @Query("update UserPage u set u.status = :status where u.name = :name")
    ...
    @Query(value = "SELECT u FROM UserPage u WHERE u.name IN :names")
    List<User> findUserByNameList(@Param("names") Collection<String> names);
    
    void deleteAllByCreationDateAfter(LocalDate date);
    
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("update UserPage u set u.active = false where u.lastLoginDate < :date")
    void deactivateUsersNotLoggedInSince(@Param("date") LocalDate date);
    
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("delete UserPage u where u.active = false")
